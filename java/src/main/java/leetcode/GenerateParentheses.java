...
final class GenerateParentheses {
     ...
     * Return all all combinations of well-formed parentheses.
     *
     * @param candidate the number of pairs of parentheses
     * @param n         the number of pairs of parentheses
     * @param result    the List<String> of all well-formed parentheses
     */
    private static void recursiveHelper(final String candidate, final int n,
        ...
        if (candidate.length() == 2 * n) {
            ...
            if (leftCount == rightCount) { // Can open more if we have the same
                                           // number (haven't hit 2*N)
                recursiveHelper(candidate + "(", n, result);
            } else if (leftCount > rightCount) {
                if (leftCount < n) { // Can open more if we haven't hit length
                                     // yet
                    recursiveHelper(candidate + "(", n, result);
                }
